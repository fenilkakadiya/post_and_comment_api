...
router.post('/', (req, res) => {
    ...
    if (!admin_id || !password) {
        return res.status(400).json({ error: 'admin_id and password are compulsory' });
    }

    con.query('SELECT * FROM admin  WHERE admin_id = ? AND password = ?', [admin_id, password], (err, results) => {
        if (err) {
